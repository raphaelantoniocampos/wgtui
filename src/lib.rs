...
///
/// Checks elevation explicitly with a real `throw`, before anything else
/// runs, rather than relying on `Get-WindowsUpdate`'s own error handling for
/// the unelevated case: confirmed live on a real machine, `Get-WindowsUpdate`
/// throws `AccessDenied` when not elevated, but that's a *non-terminating*
/// PowerShell error by default and doesn't reliably respect `-ErrorAction
/// Stop` either — a plain `throw` (confirmed live to reliably abort a
/// `-Command` chain) sidesteps its error handling entirely rather than
/// trusting it.
///
/// Then ensures PSWindowsUpdate is present (`-Scope CurrentUser`; installing
/// the *module* itself doesn't need Administrator, only `Get-WindowsUpdate`
/// does — mirrors `bootstrap.rs`'s `BOOTSTRAP_PS`), then runs `-AcceptAll
/// -Install -IgnoreReboot`: accepts every pending update and installs it,
/// but never reboots the machine on its own.
///
/// Deliberately does *not* mirror a `Set-ExecutionPolicy -Scope LocalMachine`
/// bookending approach: that needs Administrator just to run, and has a real
/// bug — if `Get-WindowsUpdate` throws partway through, a line restoring the
/// policy at the end would never run, permanently altering the machine's
/// execution policy. The process-scoped `-ExecutionPolicy Bypass` flag
/// passed to `powershell.exe` (see `run_command_stdout` call sites) achieves
/// the same thing without needing admin or mutating any persistent state —
/// already the established pattern in this codebase (`bootstrap.rs`'s
/// `BOOTSTRAP_PS`).
...
";
    ...
    fn windows_update_install_ps_checks_elevation_explicitly_first() {
        // PSWindowsUpdate's own error handling for the unelevated case isn't
        // reliable enough to depend on, so check explicitly and fail fast
        // with a clear message before even touching the module.
        assert!(
